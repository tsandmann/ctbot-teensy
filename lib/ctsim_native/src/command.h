...
public:
    ...
    CommandBase(boost::asio::streambuf& buf);
    ~CommandBase() {}
    ...
    void append_payload(boost::asio::streambuf& buf, const size_t len);
...
public:
    ...
    Command(const CommandCodes& cmd_code, const CommandCodes& subcmd_code, int16_t data_l, int16_t data_r, uint8_t from = ADDR_NOT_SET, uint8_t to = ADDR_NOT_SET) :
...
inline CommandData::CommandData() : startCode { static_cast<uint8_t>(CommandCodes::CMD_STARTCODE) }, command { 0 },
    subcommand { static_cast<uint8_t>(CommandCodes::CMD_SUB_NORM) }, direction { static_cast<uint8_t>(CommandDirection::CMD_REQUEST) },
    payload { 0 }, data_l { 0 }, data_r { 0 }, seq { 0 }, from { Command<>::ADDR_NOT_SET }, to { Command<>::ADDR_NOT_SET }, CRC { static_cast<uint8_t>(CommandCodes::CMD_STOPCODE) } {
}

inline CommandData::CommandData(const CommandCodes& cmd_code, const CommandCodes& subcmd_code, int16_t data_l, int16_t data_r, uint8_t from = Command<>::ADDR_NOT_SET) :
    startCode { static_cast<uint8_t>(CommandCodes::CMD_STARTCODE) }, command { static_cast<uint8_t>(cmd_code) },
    subcommand { static_cast<uint8_t>(static_cast<uint8_t>(subcmd_code) & 0x7f) }, direction { static_cast<uint8_t>(CommandDirection::CMD_REQUEST) }, payload { 0 },
    data_l { data_l }, data_r { data_r }, seq { 0 }, from { from }, to { Command<>::ADDR_NOT_SET },
    CRC { static_cast<uint8_t>(CommandCodes::CMD_STOPCODE) } {
}
