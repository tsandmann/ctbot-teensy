...
class PWMServo {
private:
    static constexpr uint8_t NO_ANGLE { 0xff };

    uint8_t pin;
    uint8_t angle; // in degrees
    uint16_t min16; // minimum pulse, 16uS units  (default is 34)
    uint16_t max16; // maximum pulse, 16uS units, 0-4ms range (default is 150)
// #if defined(__AVR__)
...
public:
    ...
    uint8_t attach(const uint8_t pinArg) {
        // pulse length for 0 degrees in microseconds, 544uS default
        // pulse length for 180 degrees in microseconds, 2400uS default
        return attach(pinArg, 544U, 2400U);
    }

    // attach to a pin, sets pinMode, returns false on failure, won't position the servo until a subsequent write() happens
    bool attach(const uint8_t pinArg, const uint16_t min, const uint16_t max);

    void detach();

    void write(const int angleArg); // specify the angle in degrees, 0 to 180
