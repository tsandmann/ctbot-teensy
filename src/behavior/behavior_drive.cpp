...
BehaviorDrive::BehaviorDrive(const int16_t distance, const uint16_t priority)
    : Behavior { "DriveBeh", priority, DEFAULT_CYCLE_TIME, STACK_SIZE }, state_ { State::DRIVE }, start_ {}, distance_2_ { distance * distance }, forward_ {
          distance >= 0
      } {
    ...
    /* save current pose */
    start_ = *get_pose();
    debug_printf<DEBUG_>(PP_ARGS("BehaviorDrive::BehaviorDrive(): start_="));
    if (DEBUG_) {
        start_.print(*get_ctbot()->get_comm());
    }
    debug_printf<DEBUG_>(PP_ARGS("\n\rBehaviorDrive::BehaviorDrive(): forward_={}\r\n", forward_));
}

BehaviorDrive::~BehaviorDrive() {
    debug_printf<DEBUG_>("BehaviorDrive::~BehaviorDrive()\r\n");
    debug_flush<DEBUG_>();
    ...
    switch (state_) {
        ...
        case State::END:
            ...
            if (get_speed()->get_left() == 0 && get_speed()->get_right() == 0) {
                ...
                if (get_speed()->get_left() == 0 && get_speed()->get_right() == 0) {
                    debug_print<DEBUG_>("BehaviorDrive::run(): done.\r\n");
                    if (DEBUG_) {
        ...
        case State::ABORT:
            debug_print<DEBUG_>("BehaviorDrive::run(): aborted.\r\n");
            debug_flush<DEBUG_>();
            exit();
            return;
    }
