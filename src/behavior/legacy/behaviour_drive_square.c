...
static uint8_t check_wall(void) {
    if (sensDistL < 400 || sensDistR < 400) {
        LOG_DEBUG("check_wall(): wall found: L=%d, R=%d", sensDistL, sensDistR);
        return 1;
    }
...
void bot_drive_square_behaviour(Behaviour_t* data) {
    switch (state) {
        case STATE_FORWARD:
            /* Vorwaerts */
            LOG_DEBUG("bot_drive_distance(%d)", len / 10);
            bot_drive_distance(data, 0, BOT_SPEED_FOLLOW, len / 10);
            bot_cancel_behaviour(data, bot_drive_distance_behaviour, check_wall);
            state = STATE_WAIT_1;
            break;
...
void bot_drive_square_behaviour(Behaviour_t* data) {
    switch (state) {
        ...
        default: return_from_behaviour(data); break;
    }
}
