 ...
 * @author  anonybot (anonybot@riseup.net), based on template "behaviour_prototype.h" by Benjamin Benz, created with a lot of help by Timo Sandmann
 * @date    30.11.2019
 */
...
static uint8_t cancel_follow_line_object_catched(void) {
    ...
    if (sensTrans == 1) {
        //"1" bedeutet, dass ein Objekt im Transportfach ist und die Abbruch-Funktion mittels "return" meldet, dass die Abbruch-Bedingung erfuellt ist und das
        // Verhalten, einer Linie zu folgen, abgebrochen werden soll
        return 1;

    } else {
        // ansonsten, also wenn sich nichts im Transportfach befindet, meldet die Abbruch-Funktion, dass die Abbruch-Bedingung nicht erfuellt ist und das
        // Verhalten, einer Linie zu folgen, weiterlaufen muss
        return 0;
    }
}
...
static uint8_t cancel_follow_line_on_border(void) {
    ...
    if (sensBorderL > 600 && sensBorderR > 600) {
        ...
        // ansonsten, sofern also die Kanten-Sensoren keine schwarze Linie sehen, meldet die Abbruch-Funktion, dass die Bedingung nicht erfuellt ist und das
        // Verhalten, einer Linie zu folgen, weiterlaufen muss
        return 0;
    }
}
...
void bot_adventcal_behaviour(Behaviour_t* data) {
    switch (adventcal_state) {
        ...
        case STATE_ADVENTCAL_START:
            ...
            bot_servo(data, SERVO_1, DOOR_OPEN);
        ...
        case STATE_ADVENTCAL_TURNTOSTARTPOS:
            ...
            // nicht ganz 180 Grad, damit das Liniensuchverhalten weniger dazu tendiert, den Bot beim Start am naechsten Tag nicht in die falsche Richtung zu
            // drehen, sodass keine ideale Neuausrichtung des Bots noetig ist
            bot_turn(data, 170);
